...
test.describe("atrial fibrillation video page testing", () => {
  ...
  test("ALT-06 Verify the appointment page", async ({ page }) => {
    ...
    await expect(atrialFibrillationVideoPage.appointmentHeader).toHaveText(
      " ابحث عن طبيب واحجز موعد بكل سهولة "
    );
  });
  ...
  test("ALT-07 Verify the QnA page", async ({ page }) => {
    let atrialFibrillationVideoPage = new AtrialFibrillationVideoPage(page);
    await expect(
      atrialFibrillationVideoPage.viewAllQuestionsLink
    ).toBeVisible();
    ...
    await expect(atrialFibrillationVideoPage.qnaHeader).toHaveText(
      "أسئلة واجابات طبية الامراض المعدية"
    );
  });
  ...
  test("ALT-08 Verify news and articles container", async ({ page }) => {
    ...
    let arrNewsArticles = await page.$$(
      "#suggested-articles-news .article-primary-container"
    );
    for (let i = 0; i < arrNewsArticles.length; i++)
      await expect(
        atrialFibrillationVideoPage.newsArticle.nth(i)
      ).toBeVisible();
  });
  ...
  test("ALT-10 Verify Video articles container", async ({ page }) => {
    ...
    for (let i = 0; i < arrVideoArticles.length; i++)
      await expect(
        atrialFibrillationVideoPage.videoArticle.nth(i)
      ).toBeVisible();
  });
});
